...
export const DashboardChart = ({ thegraphData, prismicData, days }: any) => {
    const { Chart, ChartItem, itemProps, tooltipProps } =
        chartComponents[prismicData?.chartType] || {};
    ...
    const totalCount = (a: any, b: any) => {
        return (
            Math.round(
                (parseFloat(a !== undefined ? a : 0) +
                    parseFloat(b !== undefined ? b : 0)) *
                    100
            ) / 100
        );
    };
    ...
    return (
        <Card>
            ...
            <Text bold extralarge fLayout="end start" inlineFlex>
                {updatedGraphData().reduce(
                    (a: any, b: any) => totalCount(a, b),
                    0
                )}
            ...
            <ResponsiveContainer height={chartDefaultHeight} width="100%">
                ...
                >
                    <Tooltip
                        {...tooltipProps}
                        content={<CustomTooltip prismicData={prismicData} />}
                    />
