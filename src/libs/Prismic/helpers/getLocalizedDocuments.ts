...
export const getLocalizedDocument = (
    documents: PrismicDocument[],
    lang: string
): PrismicDocument => {
    const defaultDocument = documents.find(
        ({ lang: documentLang }) =>
            documentLang.toLowerCase() === defaultLang.toLowerCase()
    );
    ...
    const localizedDocument = documents.find(
        ({ lang: documentLang }) =>
            documentLang.toLowerCase() === lang.toLowerCase()
    );

    return localizedDocument || defaultDocument;
};

export const getLocalizedDocuments = (
    documents: PrismicDocument[],
    lang: string
): PrismicDocument[] => {
    ...
    const localizedDocuments = defaultDocuments.map(
        (document: PrismicDocument) => {
            const { id: localizedDocumentId } =
                document?.alternate_languages.find(
                    ({ lang: alternateLang }) =>
                        alternateLang.toLowerCase() === lang.toLowerCase()
                ) || {};

            const selectedDocument = documents.find(
                ({ id }) => id === localizedDocumentId
            );
