...
const Actions = ({ buttonLoading, community, updateReview, thegraphData }: any) => {
    const { t } = useTranslations();
    const { user } = useSelector(selectCurrentUser);

    return (
        <Box flex fLayout={{ sm: 'center end', xs: 'center start'}} style={{ gap: '1rem' }} fWrap={{ sm: 'nowrap', xs: 'wrap' }}>
            ...
            {((community?.review === 'pending' || community?.review === 'declined') && !!user?.ambassador && community?.review !== 'declined') && (
                <Box>
                    <Button
            ...
            {(!!user?.councilMember && community?.contractAddress) &&
                <Box>
                    <Button error onClick={() => openModal('removeCommunity', { community, thegraphData })}>
