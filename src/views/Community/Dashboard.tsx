...
const Dashboard = ({ data: thegraphData, daysInterval }: any) => {
    ...
    const { heading: prismicHeading, rows: prismicData, text: prismicText } = extractFromView('dashboard') as any;

    // Add days to emptyDailyEntities
    const emptyDailyEntitiesWithDays: any[] = []
    
    for (let index = 0; index < daysInterval; index++) {
        emptyDailyEntitiesWithDays.push({...emptyDailyEntities, dayId: (Math.floor(new Date().getTime()/1000/86400) - daysInterval) + index})
    }

    // Add empty dailyEntities when days don't have data (thegraph only sends data the day a change ocurs)
    const newThegraphData = () => {
        const dayArray = thegraphData?.map((data: any) => { return ( data?.dayId )})
    ...
    const itemArray = (helper: string) => {
        const itemArray = newThegraphData()?.map((data: any) => { return ( data[helper] )})
    ...
    const dayArray = () => {
        const dayArray = newThegraphData()?.map((data: any) => { return ( data?.dayId )})
    ...
    return (
        <>
            ...
            <Box mt={4}>
                ...
                {!!prismicText?.length && (
                    <RichText content={prismicText} />
                )}
            </Box>

            <Box mt={2}>
                {/* Rows: Distribution, Fundraising, etc */}
                {prismicData?.map((row: { primary: { heading: string; text: string }; items: any[]; }, rowIndex: React.Key) => (
                    <Box key={rowIndex} mt={rowIndex && 3}>
                        ...
                        {!!row?.primary?.text?.length && (
                            <RichText content={row?.primary?.text} g600 mt={0.5} />
                        )}
                            ...
                            {row?.items?.map((item, chartKey) => 
                                <Box key={chartKey}>
                                    <DashboardChart 
                                        days={dayArray()}
                                        prismicData={item}
                                        thegraphData={itemArray(item.chartHelper)}
                                    /> 
                                </Box>
                            )}
                        </Grid>
