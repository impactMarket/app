...
const schema = yup.object().shape({
    children: yup.number().positive().integer().min(0).max(20).nullable(true).transform((_, val) => val === '' ? null : Number(val))
});
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Box pl={1.5} pr={1.5}>
                <Input 
                    control={control}
                    hint={errors?.children ? t(errors?.children?.message?.key)?.replace('{{ value }}', errors?.children?.message?.value) : ''}
