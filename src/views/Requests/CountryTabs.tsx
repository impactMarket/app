...
const CountryTabs = ({
    ...
    const myCountryCommunities = () => {
        ...
        !!Object.keys(allCountries).length &&
            allCountries
                .filter(
                    (countryName: any) => countryName?.country === userCountry
                )
                .map((country: any) => {
                    myCountryCommunities =
                        parseInt(country?.pending, 10) +
                        parseInt(country?.claimed, 10) +
                        parseInt(country?.declined, 10);
                });
    ...
    const otherCountriesCommunities = () => {
        ...
        !!Object.keys(otherCountries).length &&
            otherCountries.map((country: any) => {
                otherCountriesCommunities.push(
                    parseInt(country?.pending, 10) +
                        parseInt(country?.claimed, 10) +
                        parseInt(country?.declined, 10)
                );
    ...
    const handleClickOnCountryFilter = (
        countryFilter: any,
        myCountrySelected: any
    ) => {
        ...
        >
            <TabList>
                <Tab
                    ...
                    onClick={() =>
                        handleClickOnCountryFilter('mycountry', true)
                    }
                ...
                <Tab
                    ...
                    onClick={() =>
                        handleClickOnCountryFilter('othercountries', false)
                    }
