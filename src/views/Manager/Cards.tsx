...
const renderCard = (card: Cards, index: number, type: number) => {
    const titleProps =
        type === 1
            ? { g900: true, medium: true }
            : { g500: true, medium: true, small: true };
    const numberProps =
        type === 1
            ? { g900: true, large: true, mt: 1.5, semibold: true }
            : { g900: true, mt: 0.5, semibold: true, small: true };
    const colProps =
        type === 1 ? { lg: 4, md: 3, sm: 6, xs: 12 } : { md: 3, sm: 6, xs: 12 };
            ...
            >
                <Box pl={1.5} pr={1.5} w="100%">
                    <Text {...titleProps} flex>
                        {card.title}
                        {card?.tooltip && (
                            <TooltipIcon variables={card?.tooltipVariables}>
                                {card?.tooltipContent}
                            </TooltipIcon>
                        )}
                    </Text>
                    <Display {...numberProps}>{card.number}</Display>
                </Box>
                <Box w="100%">
                    <Divider />
...
const Manager: React.FC<{
    communityAmbassador: PutPostUser;
    primaryCards: Cards[];
        ...
        <>
            <Row colSpan={1.5} margin={0} mt={2} w="100%">
                {primaryCards?.length > 0 &&
                    primaryCards.map((card: any, index: number) =>
                ...
                <Col colSize={{ lg: 4, md: 6, sm: 12, xs: 12 }}>
                    ...
                    >
                        ...
                        >
                            ...
                            <Box column flex>
                                {(communityAmbassador?.firstName ||
                                    communityAmbassador?.lastName) && (
                                    <Text g700 semibold>
                        ...
                        <Box w="100%">
                            ...
                            <Box fLayout="end" flex pl={1.5} pr={1.5}>
                                ...
                                <TextLink disabled href="#" medium p700 small>
