...
const Home: React.FC<{ isLoading?: boolean }> = (props) => {
    ...
    useEffect(() => {
        ...
        const rolesCards = userRoles?.map(
            (user: string) =>
                prismic[`cards${user.charAt(0).toUpperCase() + user.slice(1)}`]
        );
        const rolesLinks = userRoles?.map(
            (user: string) =>
                prismic[`links${user.charAt(0).toUpperCase() + user.slice(1)}`]
        );
        ...
        if (user && (cards?.length !== 0 || links?.length !== 0)) {
            ...
            setCards(
                mergedCards.filter((obj, index) => {
                    return (
                        index ===
                        mergedCards.findIndex(
                            (o) => obj.title === o.title && obj.url === o.url
                        )
                    );
                })
            );
            setLinks(
                mergedLinks.filter((obj, index) => {
                    return (
                        index ===
    ...
    return (
        <ViewContainer {...({} as any)} isLoading={isLoading}>
            <Row>
                <Alerts />
                {cards?.map(
                    (card: any, key: React.Key) =>
                        card.isActive &&
                        card.title &&
                        card.url && (
                            <Col
                                colSize={{ sm: card.size ? 12 : 6, xs: 12 }}
                                padding={0.5}
                                key={key}
                            >
